  ...
  return (
    <div className="relative hidden w-full sm:block sm:max-w-[220px] lg:max-w-[350px] group transition-all duration-300">
      ...
      {debouncedQuery.length > 2 && query !== "" && (
        <div className="absolute top-12 right-0 w-[min(90vw,24rem)] bg-card/95 backdrop-blur-xl border border-border/50 rounded-2xl shadow-2xl z-50 p-4 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex items-center gap-2 px-2 border-b border-border/30 pb-2">
            <Globe className="h-3 w-3 text-primary" />
            <span className="text-xs font-black uppercase tracking-wider text-muted-foreground">Resultados Globales</span>
          </div>

          {isLoading ? (
            <div className="py-8 flex flex-col items-center justify-center gap-2">
              <Loader2 className="h-6 w-6 animate-spin text-muted-foreground/30" />
              <span className="text-xs font-bold text-muted-foreground uppercase">Explorando registros...</span>
            </div>
            ...
            <div className="flex flex-col gap-1 max-h-[300px] overflow-y-auto custom-scrollbar">
              {results.data.map((item: any) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground line-clamp-1">{item.descripcion || 'Sin descripción'}</span>
                </button>
              ))}
