...
export const enfermedadesService = {
  ...
  getTipos: async (): Promise<TipoEnfermedadResponse> => {
    const response = await apiClient.get<TipoEnfermedadResponse>('/t_enfermedad');
    return response.data;
  },

  createTipo: async (nombre: string): Promise<TipoEnfermedadResponse> => {
    const response = await apiClient.post<TipoEnfermedadResponse>('/t_enfermedad', { nombre });
    return response.data;
  },

  updateTipo: async ({ id, nombre }: { id: number; nombre: string }): Promise<TipoEnfermedadResponse> => {
    const response = await apiClient.put<TipoEnfermedadResponse>(`/t_enfermedad/${id}`, { nombre });
    return response.data;
  },

  deleteTipo: async (id: number): Promise<SimpleResponse> => {
    const response = await apiClient.delete<SimpleResponse>(`/t_enfermedad/${id}`);
    return response.data;
  },
