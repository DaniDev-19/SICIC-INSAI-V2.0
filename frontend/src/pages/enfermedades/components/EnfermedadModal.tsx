...
import type { Enfermedad, TipoEnfermedad } from '@/types/enfermedades';
import { Stethoscope, Loader2, Plus, Pencil, Trash2, Check, X } from 'lucide-react';
...
const enfermedadSchema = z.object({
  ...
  zoonatica: z.string().nullable().optional(),
  descripcion: z.string().nullable().optional(),
  tipo_enfermedad_id: z.string().min(1, 'El tipo de enfermedad es requerido'),
});

type EnfermedadFormValues = z.infer<typeof enfermedadSchema>;

interface EnfermedadModalProps {
  isOpen: boolean;
  onClose: () => void;
  enfermedad?: Enfermedad | null;
  tipos: TipoEnfermedad[];
...
export function EnfermedadModal({
  isOpen,
  onClose,
  enfermedad,
  tipos,
  ...
  onUpdateTipo,
  onDeleteTipo,
}: EnfermedadModalProps) {
  const { createEnfermedad, updateEnfermedad, isCreating, isUpdating } = useEnfermedades();
  ...
  useEffect(() => {
    if (enfermedad) {
      reset({
        ...
        nombre_cientifico: enfermedad.nombre_cientifico || '',
        zoonatica: enfermedad.zoonatica || '',
        descripcion: enfermedad.descripcion || '',
        tipo_enfermedad_id: enfermedad.tipo_enfermedad_id?.toString() || '',
      });
    } else {
      reset({
        nombre: '',
  ...
  }, [enfermedad, reset, isOpen]);

  const handleFormSubmit = async (values: EnfermedadFormValues) => {
    const cleanData = {
      ...values,
      tipo_enfermedad_id: parseInt(values.tipo_enfermedad_id),
    };

    if (enfermedad) {
      await updateEnfermedad({ id: enfermedad.id, enfermedad: cleanData });
    } else {
      await createEnfermedad(cleanData as any);
    }
    onClose();
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <DialogTitle className="text-2xl font-bold">
                {enfermedad ? 'Editar Enfermedad' : 'Nueva Enfermedad'}
              </DialogTitle>
              <p className="text-sm text-muted-foreground mt-1">
                {enfermedad ? 'Modifica la información de la enfermedad' : 'Registra una nueva enfermedad en el sistema'}
              </p>
            </div>
          </div>
        </DialogHeader>
          ...
          <div className="space-y-5 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ej. Aphthovirus"
                className="h-12 rounded-xl border-border bg-muted/10 focus:bg-background transition-all italic"
              />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest pl-1 mb-1 block">Zoonótica</label>
              <Input
                {...register('zoonatica')}
                placeholder="Ej. Sí, No, Potencial"
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest pl-1 mb-1 block">
                Tipo de Enfermedad <span className="text-rose-500">*</span>
              </label>
              ...
              <div className="flex items-center gap-2">
                <Select onValueChange={(val) => setValue('tipo_enfermedad_id', val, { shouldValidate: true })} value={watch('tipo_enfermedad_id')}>
                  <SelectTrigger className={cn(
                    "w-full h-12 rounded-xl border-border bg-muted/10 focus:bg-background transition-all",
                    errors.tipo_enfermedad_id && "border-rose-500/50 bg-rose-500/5 focus:border-rose-500"
                  )}>
              ...
              {errors.tipo_enfermedad_id && <p className="text-[10px] text-rose-500 font-bold uppercase tracking-wider pl-1 animate-in fade-in slide-in-from-left-1">{errors.tipo_enfermedad_id.message}</p>}
            </div>
          ...
          <DialogFooter className="pt-4 pb-2">
            ...
            <Button type="submit" disabled={isLoading} className="rounded-xl h-12 px-8 shadow-lg shadow-primary/20 bg-primary hover:shadow-primary/40 transition-all font-bold cursor-pointer">
              {isLoading ? (
                ...
                enfermedad ? 'Guardar Cambios' : 'Registrar Enfermedad'
              )}
            </Button>
          </DialogFooter>
        </form>
