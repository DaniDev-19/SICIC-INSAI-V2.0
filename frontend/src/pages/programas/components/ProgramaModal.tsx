...
const programaSchema = z.object({
  ...
  tipo_programa_id: z.string().optional().nullable(),
});
...
export function ProgramaModal({
  ...
  const watchTipo = watch('tipo_programa_id');
    ...
    if (isOpen) {
      if (programa) {
        reset({
          ...
          tipo_programa_id: programa.tipo_programa_id ? programa.tipo_programa_id.toString() : '',
        });
      } else {
        reset({
          nombre: '',
  ...
  const onFormSubmit = async (values: any) => {
    const cleanData = {
      ...
      tipo_programa_id: values.tipo_programa_id ? parseInt(values.tipo_programa_id) : undefined,
    };

    if (programa) {
      await updatePrograma({ id: programa.id, data: cleanData as any });
    } else {
      await createPrograma(cleanData as any);
    }
    onClose();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[calc(100vw-1.5rem)] sm:max-w-[500px] glass-effect border-border shadow-2xl rounded-3xl overflow-hidden p-0 flex flex-col max-h-[min(92vh,48rem)]">
        <form onSubmit={handleSubmit(onFormSubmit)}>
          ...
          <div className="p-8 space-y-6">
            <div className="space-y-2">
              ...
              />
              {errors.nombre && <span className="text-xs text-rose-500 font-medium ml-1">{errors.nombre.message as string}</span>}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Select
                  value={watchTipo || ''}
                  onValueChange={(value) => setValue('tipo_programa_id', value, { shouldValidate: true })}
                >
